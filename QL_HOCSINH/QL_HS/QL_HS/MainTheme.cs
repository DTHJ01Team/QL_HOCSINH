    ...
    public partial class MainTheme : DevComponents.DotNetBar.Office2007RibbonForm
    {
        ...
        private void loadData()
        {
            try
            {
                SqlConnection URL = new SqlConnection(link);
                URL.Open();             
                string sql = "SELECT MAHS as[Mã hs], Hoten AS [Họ tên], MAHK as [Mã học kỳ], MAMH as [Mã môn], Diem15 as [Điểm 15'], Diem45 as[Điểm 45'] FROM DIEMKIEMTRA";
                SqlCommand commandSql = new SqlCommand(sql, URL); // Thực thi câu lệnh SQL
                SqlDataAdapter com = new SqlDataAdapter(commandSql); //Vận chuyển dữ liệu
                DataTable table = new DataTable();
                com.Fill(table);
        ...
        private void btndSua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection URL = new SqlConnection(link);
                URL.Open();
                string sua = "UPDATE NHAPDIEM SET MAHS='" + cbndMahocsinh.SelectedValue.ToString() + "', Hoten = '"+txtndHoten.Text+"', MAHK=N'" + HK + "', MAMH='" + cbndMamon.SelectedValue.ToString() + "', Diem15=N'" + txtndDiem15.Text + "' ,Diem45 = '"+txtndDiem45.Text+"' WHERE MAHS='" + cbndMahocsinh.SelectedValue.ToString() + "'";
                SqlCommand command = new SqlCommand(sua, URL);
                command.ExecuteNonQuery();
                loadData();
            }
        ...
        private void btndXoa_Click(object sender, EventArgs e)
        {
            try
            {
                SqlConnection URL = new SqlConnection(link);
                URL.Open();
                string xoa = "DELETE FROM NHAPDIEM WHERE MAHS = '" + cbndMahocsinh.SelectedValue + "'";
                SqlCommand command = new SqlCommand(xoa, URL);
                command.ExecuteNonQuery();
                loadData();
            }
